// file contour_noK_k.c to run recurrence equations of ODEs and output to a file for upload and analysis; accompanies Wilcox et al.:  Phage T7 adaptation to two antibiotics
// Here adsorption rate (k) is varied to change phage growth rate (Fig. 5B)
...
printf("input b,  lysis rate, decay rate\n");
scanf("%lf %lf %lf",&b,&L,&delta);
printf("data input\n");
 
fprintf(fp,"b = %2.1f, lysis rate = %1.4f decay rate = %1.3f\n",b,L,delta);
fprintf(fp,"growth rate, cells, Pf0\n");


for(B0 = 4.0;B0<8.5; B0+= 0.2)				{
for(lk = -12.0;lk<-8;lk+=0.2)		{
